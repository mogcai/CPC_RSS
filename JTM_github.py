...
def get_jtm_post_list(page=1):
    ...
    if r.status_code == 200:
        ...
        logging.info("✅ 連線成功。")
        soup = BeautifulSoup(r.content.decode("utf-8"), parser="html.parser")
...
def get_article(url):
    ...
    if date_tag := soup.find("span", {"class": "meta-date"}):
    ...
    if img_tag := soup.find("div", {"class": re.compile("post-image")}):
        img_url = img_tag.img.get("src")

    if author_tag := soup.find("span", {"class": re.compile("meta-author")}):
        author = author_tag.text
        author = author if author != "" else "None"
    else:
        author = None

    if title_tag := soup.find("h2", {"class": re.compile("single-title")}):
        title = title_tag.text
    else:
        title = None

    if content_tag := soup.find(
        "div", {"class": re.compile("post-content entry clearfix")}
    ):
