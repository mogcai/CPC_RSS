...
def get_article(url):
    ...
    if date_tag := soup.find("time", {"class": "entry-date updated td-module-date"}):
    ...
    if img_tag := soup.find("img", {"class": re.compile("entry-thumb")}):
        img_url = img_tag.get("src")

    if title_tag := soup.find("h1", {"class": re.compile("title")}):
    ...
    if content_tag := soup.find("div", {"class": re.compile("post-content")}):
    ...
    if cat_tag := soup.find("a", {"class": "tdb-entry-category"}):
...
def scrape_ponto_final(today):
    ...
    if r.status_code == 200:
        ...
        content_tag = soup.find_all(
            "div", class_="td-main-content-wrap td-container-wrap"
        )
        if content_tag:
            content = content_tag[0]
            posts = content.find_all("h3", class_="entry-title td-module-title")
            valid_post = list(
                set([post.find_all("a")[0].get("href") for post in posts])
            )
        else:
            valid_post = []
...
valid_post = []
for taget_date in [today, yesterday]:
    valid_post += scrape_ponto_final(taget_date)
