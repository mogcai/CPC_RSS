...
target_dates=[today]
# target_dates=[(datetime.today()-timedelta(days=3)).strftime('%Y/%m/%d')]
valid_post=[]
for target_date_str in target_dates:
    ...
    if r.status_code==200:
        ...
        content_tag=soup.find('div', class_='td-pb-span8 td-main-content')
        if content_tag:
            ...
            logging.info(f"✅ 成功獲取 {target_date_str} 數據，當日有{total_no_post}條新聞。")
        else:
...
def get_article(url):
    ...
    try:
        ...
        if r.status_code==200:
            ...
            author_tag=data.find('a', {'class': 'tdb-author-name'})
            if author_tag:
            ...
            img_url = ""
            img_tag = data.find('div', {'class': re.compile('td-post-featured-image')})
            if img_tag:
                img_url = img_tag.img.get('src')

            content_tag=data.find('div', class_='td-post-content')
            if content_tag:
            ...
            cat_tag=data.find('li', {'class': 'entry-category'})
            if cat_tag:
